...
export default function ProfileScreen({ onNavigate, reflections, aiPlans, user, signOut, isDesktop }: {
  ...
  return (
    <div
      className={isDesktop ? 'relative size-full' : 'overflow-hidden relative rounded-[36px] size-full flex flex-col'}
    ...
    >
      ...
      {/* Scrollable content */}
      <div
        className={isDesktop ? 'absolute top-0 left-0 right-0 bottom-0 overflow-y-auto' : 'relative z-[1] flex-1 overflow-y-auto'}
        style={{ scrollbarWidth: 'none' }}
      >
        {/* Centering wrapper on desktop */}
        <div className={isDesktop ? 'max-w-[560px] mx-auto' : ''}>
        {/* Header */}
        <div className={isDesktop ? 'pt-[28px] px-[24px]' : 'pt-[56px] px-[17px]'}>
      ...
      {!isDesktop && (
        <div
          className="relative z-20 shrink-0 border-t"
          style={{
        ...
        >
          ...
          <div className="flex items-center justify-center pb-[6px]">
            <div className="h-[4px] rounded-[2px] w-[100px]" style={{ backgroundColor: isDark ? '#333333' : '#c8d4de' }} />
          </div>
        </div>
      )}
    </div>
